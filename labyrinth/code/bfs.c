...
// Generates the vector of positions next to the given one, taking into account the walls of the labyrinth
// The positions are generated in the format of a number, according to the instructions, as it's easier to store. 
Vector_t generate_neighbours(Vector_t dimensions, uintmax_t position, bool *error) {
    uintmax_t move = 1; 
    uintmax_t cut_down_position = position; // position divided product of dimensions
...
// Filters the given moves based on whether they are possible in the given the layout of the labyrinth
void filter_neighbours(Bitmap_t labyrinth, Vector_t neighbours) {
    for (size_t i = 0; i < the_size_of_array(neighbours); i++) {
        ...
        if (bit) {
            ...
            i--; // The elements are removed online, so we need to adjust which element we're considering
        }
    }
}
...
void breadth_first_search(Setup input) {
    ...
    while (!error_flag && !found_flag) {
        // Parity of the step determines which queue contains the old states vs the new ones
        Queue_t active_queue = (length_of_path % 2 ? a_queue : b_queue);
        Queue_t receiving_queue = (length_of_path % 2 ? b_queue : a_queue);

        // This loop is one step in a search: expands all current states into new ones
        while (!queue_is_empty(active_queue) && !error_flag && !found_flag) {
            uintmax_t position = take_from_queue(active_queue);
            Vector_t neighbours = generate_neighbours(input->dimensions, position, &error_flag);
            ...
            for (size_t i = 0; i < the_size_of_array(neighbours); i++) {
                ...
                // If we found are destination, there's no point in continuing the search
                if (candidate == input->exit) {
                    printf("%"PRIuMAX"\n", length_of_path);
                    found_flag = true;
                    break;
                }
                // Since we don't want to check the places we've already been to, we might as well
        ...
        // If the queues were reallocated, their address changed, so the pointers must be reset properly
        a_queue = (length_of_path % 2 ? active_queue : receiving_queue);
        b_queue = (length_of_path % 2 ? receiving_queue : active_queue);

        if (error_flag || found_flag) break;
