...
char** split_string(const char* s)
{
    ...
    fprintf(stderr, "Empty words found: %d\n", empty_words);
    char** new_parts = calloc(spaces + 2 - empty_words, sizeof(char*));
    new_parts[spaces + 1 - empty_words] = NULL;
    for (int i = 0, j = 0; i < spaces + 1; ++i) {
        if (!strcmp(parts[i], "")) {
            free(parts[i]);
        } else {
            new_parts[j] = parts[i];
