...
uintmax_t add_digit_to_number(int digit, uintmax_t number, bool *error) {
    ...
    // check if there occurred an overflow in the process - if so, sets the
    // *error flag
...
bool decimal(int digit) { return '0' <= digit && digit <= '9'; }
bool big_hexadecimal(int digit) { return 'A' <= digit && digit <= 'F'; }
bool small_hexadecimal(int digit) { return 'a' <= digit && digit <= 'f'; }
...
int hex_into_number(int digit) {
    ...
    if (decimal(digit))
        return digit - '0';
    else if (big_hexadecimal(digit))
        return digit - 'A' + 10;
    else if (small_hexadecimal(digit))
        return digit - 'a' + 10;
    else
...
// Returns true if limits exist && (sizes don't match up || at some index 'i'
// vector[i] is outside interval [1, limits[i]])
bool vector_is_outside_limits(Vector_t vector, Vector_t limits) {
    ...
    if (limit_dimension == 0)
        return false; //
    if (the_size_of_array(vector) != limit_dimension)
        return true;
...
void load_numbers_line(Vector_t vector, Vector_t limits, bool *is_error) {
    ...
    // Until the end of the line checks every individual digit and takes
    // appriopriate actions
    do {
        digit = getchar();
    ...
    if (vector_is_outside_limits(vector, limits))
        *is_error = true;
    if (!the_size_of_array(vector))
        *is_error = true;
    if (*is_error)
        report_error(line);
}
...
void load_R(uint32_t *setup_table, bool *is_error) {
    ...
    do {
        digit = getchar();
        if (decimal(digit)) { // actual digit
            number = (uint32_t)add_digit_to_number(digit - '0',
                                                   (uintmax_t)number, is_error);
            active = true;
...
// Function which loads the R version of the fourth line and generates the
// labyrinth based on it. Returns true if everything goes well, false if not
...
uintmax_t load_hex(Bitmap_t labyrinth, bool *is_error) {
    ...
    // Since we don't know how many bits we're going to receive, we don't know
    // where to put the ones we're receiving. Therefore we'll add them from the
    // end of available memory, and shift them down in the
    // 'load_hexadecimal_number' function
    uintmax_t const maxbit =
        8 * size_of_bitmap(labyrinth) - 1; // The index of the last bit

    ignore_leading_zeroes(&digit);
    // First digit needs to be considered separatly since there might be bit
    // leading zeroes
    if (significant_hex(digit)) {
        ...
        for (size_t i = 0; i < bit_count; i++) {
            ...
            if (digit2 & 8)
                set_bit_in_bitmap(maxbit - i, labyrinth);
            digit2 <<= 1;
        }
    } else {
        active = false;
    ...
    while (digit != '\n' && digit != EOF) {
        ...
        if (active && significant_hex(digit)) { // hexadecimal digit
            ...
            for (size_t i = 0; i < 4; i++) {
                if (digit & 8)
                    set_bit_in_bitmap(maxbit - bit_count, labyrinth);
                bit_count++;
                digit <<= 1;
            }
        } else if (isspace(digit)) { // white space
            active = false;
        } else if (digit == EOF) { // no action necessary, but we don't want to
                                   // trigger *is_error flag
        } else {
            *is_error = true;
            report_error(4);
        }
...
bool load_hexadecimal_number(Bitmap_t labyrinth, uintmax_t size_of_labyrinth) {
    ...
    if (error || bit_count > size_of_labyrinth)
        return false;
    if (bit_count == 0)
        return true;
...
Bitmap_t load_walls_line(bool *is_error, uintmax_t size_of_labyrinth) {
    ...
    if (digit == '0') {
        digit = getchar();
        if (digit == 'x')
            *is_error |= !load_hexadecimal_number(labyrinth, size_of_labyrinth);
        else
            *is_error = true;
    } else if (digit == 'R') {
...
static inline void emergency_exit(Vector_t v1, Vector_t v2, Vector_t v3,
                                  Setup s) {
    free_vector(v1);
    free_vector(v2);
    free_vector(v3);
...
Setup load_input() {
    ...
    // Calculates the size of the labyrinth, taking into account possible
    // overflow
